...
= 緒　　言
海事産業では世界の海難事故の約8割が人的要因で発生している他、船員の人員不足、高齢化などの課題を抱えており、これらを解消し得る手段の一つとして自動運航船の実用化に向けた研究開発が数多く行われている @kim_autonomous_2020 @burmeister_autonomous_2014 @li_risk_2023 @he_quantitative_2017 @statheros_autonomous_2008 @felski_ocean-going_2020。
自動運航船は、従来は船員が行ってきた航路追従や障害物回避などの高度な操船を機械が自律的に実行する必要がある。そのためには高度な操船制御技術に加えて、設計段階から船舶の操縦性能を適切に把握する必要があり、船舶操縦運動モデルの活用が不可欠である。船舶操縦運動モデルは船舶の航路予測や操船制御などに広く用いられており、これまでにも様々なモデルが提案されてきた @yasukawaIntroductionMMGStandard2015 @liu_predictions_2018。

従来、船舶操縦運動モデルの同定は水槽試験によるPMM（Planar Motion Mechanism）試験やCMT（Captive Model Test）試験、あるいはCFD（Computational Fluid Dynamics）解析を通じて行われてきた @kume_measurements_2006 @ueno_circular_2009 @hao_recurrent_2022 @wang_non-parameterized_2023。
これらを用いることで船舶の一つの状態における平水中の船舶操縦運動モデルを高精度に同定可能である。しかし実運航では貨物の積載状態やバラスト水の変化、運航海域によって船舶の状態が変化するため、モデル内の流体力学的なパラメータもそれに応じて変動する。これに対し、観測データから船舶の運航状態に合わせたモデルの同定手法があるが、観測データに含まれるノイズなどの不確実性により、同定された船舶操縦運動モデルは不確定なものとなる @mitsuyuki_mmg_2024。


加えて実海域では、風や波などの外乱影響が支配的となる。従来のMMGモデルなどの船舶操縦運動モデルでは、平水中の操縦運動に対して風や波などの外乱影響に起因して発生する外力を、外力項として線形に加える形で表現することが一般的である @yasukawa_application_2020 @suzuki_numerical_2021 @yasukawa_evaluations_2019 @paroka_prediction_2017。
...
一方、近年のGPSやVDR等を用いた航行計測技術の普及により、実船の座標や速度を含む観測データが豊富に取得可能となっている @vu_estimating_2023 @mei_full-scale_2020。
...
実観測データとモデルの差分から未知の外力を推定するアプローチとして、外乱オブザーバ(DOB)を用いた研究が多数存在する @gu_disturbance_2022 @menges_environmental_2023。
...
=== 観測データの作成
本項では平水中と外乱下の観測データの作成方法及び用いた対象船の概要について説明する。観測データの作成にはKCSコンテナ船の実船サイズを用いた。その主要目等については奥田らの論文に記載されている @okuda_validation_2023。
...
次に、風および波の外力を考慮した外乱下観測データを作成する。風圧力は藤原らによる推定式 @fujiwara_experimental_nodate 、波浪定常力は安川らによる推定式 @yasukawa_validation_2021 を用いて算出し、これらを真値の外力項として運動方程式の右辺に付加してシミュレーションを行った。本検証における外乱環境を @tb:disturbance_conditions に示す。なお、空間固定座標系で$x$軸正方向を0度とし、時計回りを正の向きとする。
#figure(
...
=== 提案手法のパラメータ設定
本項では環境場推定に用いるMCMC法およびモデル予測制御のパラメータ設定について述べる。平水中の観測データに対してMCMC法を適用する際に必要な各操縦流体力微係数の事前分布は先行研究 @mitsuyuki_mmg_2024 に倣い一様分布を設定した。サンプリング実施回数は1500回とし、初期の500回を破棄した。そして得られた事後分布からランダムに50個の操縦流体力微係数セットを抽出し、MPCにおける予測モデルの構築に用いた。
...
=== 提案手法のパラメータ設定
船舶操縦運動モデルの同定に用いるMCMC法のパラメータ設定にあたっては前節と同様に先行研究 @mitsuyuki_mmg_2024 に倣い設定した。MPCの設定値を @tab:mpc_params_model_test に状態変数の誤差と制御入力の変化量に対する重み行列を @eq:mpc_weights_model_test に示す。
$
  Q &= "diag"(1, 1, 1, 1)\
